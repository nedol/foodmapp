    ...
    RegUser(q, ws) {
        ...
        if (q.psw_hash) {
            ...
            let sql =
                ...
                " (SELECT data FROM offers WHERE offers.supuid = user.uid  ORDER BY published DESC  LIMIT 1) as data, " +
                " (SELECT longitude FROM offers WHERE offers.supuid = user.uid  ORDER BY published DESC  LIMIT 1) as lon, " +
                " (SELECT latitude FROM offers WHERE offers.supuid = user.uid  ORDER BY published DESC  LIMIT 1) as lat, " +
                " (SELECT radius FROM offers WHERE offers.supuid = user.uid  ORDER BY published DESC  LIMIT 1) as radius, " +
                " user.dict as dict" +
                " FROM " + q.user.toLowerCase() + " as user " +
                " WHERE " +
                " STRCMP(MD5(user.psw),'" + q.psw_hash + "')=0";
                ...
                if (ofobj[tab][item].cert){
                    for(let c in ofobj[tab][item].cert) {
                        if(ofobj[tab][item].cert[c].src.includes('base64')) {
                            ...
                            fs.writeFile(path.join(__dirname, '../images/' + hash), base64Data, 'base64', (err) => {
                            });
...
//////////////////////////////////////////////////////////////////////////////////////////////////////
        let sql = " SELECT " +
            ...
            " AND (" +
            " (SELECT id  FROM offers WHERE supuid LIKE sup.uid AND published<='"+moment(q.date).format('YYYY-MM-DD')+"' ORDER BY date DESC LIMIT 1)= of.id" +
            " OR " +
            " (SELECT id  FROM offers WHERE supuid LIKE sup.uid AND published>='"+moment(q.date).subtract(7, 'days').format('YYYY-MM-DD')+"' ORDER BY date DESC LIMIT 1)= of.id" +
            " )"+
            ...
            let promise = new Promise(function (resolve,reject) {
                ...
                if(result.length>0) {
                    ...
                    for(let o in result){
                        ...
                        for(let c in offer) {
                            if(offer[c].length===0)
                                ...
                                for (let i in offer[c]) {
                                    if(result[parseInt(o) - 1]  && result[parseInt(o) - 1].uid === result[o].uid)
                                        if(JSON.parse(result[parseInt(o) - 1].data)[c])
                                            if (JSON.parse(result[parseInt(o) - 1].data)[c][i])
                                                if (JSON.parse(result[parseInt(o) - 1].data)[c][i].packlist) {
                                                    // let key = Object.keys(offer[c][i].packlist)[0];
                                                    let x = _.findKey(JSON.parse(result[parseInt(o) - 1].data)[c], {'title': offer[c][i].title });
                                                    if(x)
                                                        offer[c][i].prev_packlist = JSON.parse(result[parseInt(o) - 1].data)[c][x].packlist;
                                                }
                                }
                            }
                        }
                        ...
                        result[o].date = q.date.split('T')[0];
                        resAr.push(result[o]);
                        //}
                    }
...
//////////////////////////////////////////////////////////////////////////////////////////////////////
        let sql =
            ...
            " AND of.latitude>="+ q.areas[0] +" AND of.latitude<="+q.areas[1] +
            " AND of.longitude>=" + q.areas[2] + " AND of.longitude<=" +q.areas[3]+
            " AND of.date>'"+moment(q.date).subtract(7, 'days').calendar() +"'"+
    ...
    translate(q, ws) {
        ...
        for(let w=0; w<Object.keys(data).length; w++) {
            ...
            let obj = {ws:ws,key:key, data:data, to:to, from:from, src:data[key][from],length:Object.keys(data).length};
    ...
    GetByKey(q, ws) {
        ...
        let sql =
            ...
            " AND LCASE(sup.profile) LIKE '%\"place\":\"%%"+q.key.toLowerCase()+"%' GROUP BY sup.uid";
        // if (!ws._header)
