...
module.exports = class Supplier extends D2D {
          ...
          setTimeout(function () {
            that.replaceImg_2(q.profile.thmb, function (thmb) {
              ...
              sql =
                'REPLACE INTO ' +
                q.user.toLowerCase() +
                ' SET  uid=?, psw=?, email=?, tariff=? ,promo=?, profile=?';
              ...
              that.mysql_con.query(sql, values, function (err, result) {
                if (err) {
                  ws.send(
                    urlencode.encode(
                      JSON.stringify({
                        func: q.func,
                        err: {
                          ru: 'Неверные данные' + JSON.stringify(err),
                          en: 'Wrong data',
                        }[q.profile.lang],
                      })
                    )
                  ...
                  let em = new Email();
                  let text = {
                    ru: '<h1>Перейдите в приложение</h1></a><p>Удачной торговли!</p>',
                    en: '<h1>Go to the app</h1></a><p>Have a fun!</p>',
                    fr: "<h1>Accéder à l'application</h1></a><p>Amusez-vous bien!</p>",
                  }[q.profile.lang];
                  let html =
                    '<a href=' +
                    q.host +
                  ...
                  em.SendMail(
                    ...
                    q.profile.email,
                    {
                      ru: 'Подтверждение регистрации пользователя',
                      en: 'User registration confirmation',
                      fr: "Confirmation de l'enregistrement de l'utilisateur",
                    }[q.profile.lang],
                    html,
                    function (result) {
                      ws.send(
                      ...
                      let values = [
                        uid,
                        q.profile.email,
                        'https://delivery-angels.ru/d2d/dist/supplier.html?lang=' +
                          q.profile.lang +
                          '&psw_hash=' +
                          md5(psw),
                      ];
                      ...
                      let sql =
                        'INSERT INTO psw_hash SET  uid=?, email=?, url=?';
                      that.mysql_con.query(
                        sql,
                        values,
                        function (err, result) {}
                      );
  ...
  UpdProfile(q, ws) {
    ...
    this.mysql_con.query(sql, function (err, result) {
      ...
      if (result.length > 0) {
        ...
        if (
          !q.profile.avatar ||
          (q.profile.avatar && !q.profile.avatar.includes('base64'))
        ) {
          values = [
            q.profile.email.toLowerCase(),
            JSON.stringify(q.profile),
            result[0].tariff,
            q.promo,
            q.uid,
          ...
          sql =
            'UPDATE ' +
            q.user.toLowerCase() +
            ' SET email=?, profile=?, tariff=?, promo=? WHERE uid=? AND psw=?';
          ...
          q.profile.avatar &&
          q.profile.avatar.includes('base64') &&
          q.profile.avatar.length < IMG_SIZE_LIMIT
        ) {
          that.replaceImg_2(
            q.profile.avatar,
            function (avatar) {
  ...
  UpdateOfferItem(q, ws) {
    ...
    this.isValidSupplier(q, ws, function (result) {
        ...
        ws.send(
          urlencode.encode(
            JSON.stringify({
              func: q.func,
              err: 'Пройдите регистрацию',
              link: 'https://delivery-angels.ru/d2d/dist/settings.supplier.en.html',
            })
          )
        );
        return;
      ...
      let sql =
        ...
        q.user.toLowerCase() +
        ' as sup, offers as of' +
        " WHERE of.supuid=sup.uid AND sup.uid='" +
        q.uid +
        "'" +
        " AND (of.date<='" +
        q.date +
      ...
      that.mysql_con.query(sql, function (err, sel) {
        ...
        if (sel.length > 0) {
          if (q.dict && q.offer) {
            // && result[0].obj_data.length<q.dict.length){
            ...
            that.replaceImg(item, function (item) {
              ...
              values = [
                ...
                q.location[1],
                q.location[0],
                JSON.stringify(catsAr),
                q.radius,
                sel[0].id,
              ];
              let sql_upd =
                'UPDATE offers SET data=?, latitude=?, longitude=?, categories=?, radius=? WHERE id=?';
          ...
          that.replaceImg(offer, function (offer) {
            ...
            values = [
              ...
              parseFloat(q.location[0]),
              parseFloat(q.location[1]),
              date,
              q.radius,
            ];

            sql =
              'REPLACE INTO offers SET supuid=?, data=?, categories=?, longitude=?, latitude=?, date=?, radius=?';
  ...
  UpdateLocation(q, ws) {
    ...
    that.mysql_con.query(sql, function (err, sel) {
      ...
      if (sel.length > 0 && moment(q.date).isSame(sel[0].date)) {
        //moment(q.date).isSame(sel[0].date)
        if (q.location) {
          // && result[0].obj_data.length<q.dict.length){

          let values = [q.location[1], q.location[0], q.radius, sel[0].id];
          let sql_upd =
            'UPDATE offers SET latitude=?, longitude=?, radius=? WHERE id=?';
          that.updateOfferDB(q, ws, sql_upd, values, now);
          if (sel[0].deliver) {
            //copy offer to deliver's offer
            //that.updateOfferDeliver(q, ws, sel[0].deliver,offer);
        ...
        sql =
          'REPLACE INTO offers SET supuid=?, longitude=?, latitude=?, date=?, radius=?,data=?';
  ...
  DeleteOffer(q, ws) {
    ...
    that.mysql_con.query(sql, function (err, res) {
      if (err) {
        throw err;
      }
        ...
        ws.send(
          urlencode.encode(
            JSON.stringify({
              ...
              err: 'Пройдите регистрацию',
              link:
                'https://delivery-angels.ru/d2d/dist/settings.' +
                q.user.toLowerCase() +
                '.en.html',
            })
          )
  ...
  updateOfferDeliver(q, ws, deliver, offer) {
    let sql =
      ...
      q.user.toLowerCase() +
      ' as del, offers as of' +
      " WHERE del.uid=of.supuid AND del.uid='" +
      deliver +
      "'" +
      " AND of.date='" +
      q.date +
      ...
      if (sel[0] && sel[0].data) {
        ...
        for (let tab in sup_obj) {
          for (let of in sup_obj[tab]) {
            if (!del_obj[tab] || !del_obj[tab][of]) continue;
  ...
  UpdateOrder(q, ws) {
    ...
    let sql =
      ...
      ' FROM   ' +
      q.user.toLowerCase() +
      ' as sup, customer as cus, orders as ord' +
      //", tariff as tar"+
      " WHERE sup.uid=ord.supuid  AND ord.supuid='" +
      q.supuid +
      "'  AND ord.cusuid='" +
      q.cusuid +
      "'" +
      ...
      " AND cus.uid=ord.cusuid AND ord.date='" +
      q.date +
      "'" +
    ...
    this.mysql_con.query(sql, (err, sel) => {
      ...
      if (sel.length > 0) {
        ...
        sql =
          'UPDATE orders SET data=?, comment=?, period=?, address=?, published=? WHERE id=?';
      } else {
        values = [
          q.cusuid,
          q.supuid,
        ...
        sql =
          'INSERT INTO orders SET cusuid=?, supuid=?, data=?, comment=?, address=?, date=?, period=?, published=?';
      }
      this.mysql_con.query(sql, values, function (err, result) {
        if (err) {
          ws.send(
        ...
        if (
          global.resObj[q.supuid] &&
          global.resObj[q.supuid].connection.writable
        ) {
          delete q.uid;
          delete q.func;
          delete q.proj;
          resObj[q.supuid].write(
            utils.formatSSE({ func: 'ordered', order: q })
          );
        }
        ws.send(
          urlencode.encode(
  ...
  ApproveOrder(q, ws) {
    ...
    this.mysql_con.query(sql, (err, sel) => {
      ...
      if (sel.length > 0) {
        ...
        sql =
          'UPDATE orders SET number=?, comment=?,  published=?, status=? WHERE id=?';
      } else {
        ws.send(
          urlencode.encode(
            JSON.stringify({
  ...
  UpdateOrderStatus(q, ws) {
    ...
    let sql =
      ...
      ' FROM   ' +
      q.user.toLowerCase() +
      ' as sup, offers as of, customer as cus, orders as ord' +
      " WHERE sup.email='" +
      q.supuid +
      "' AND sup.uid='" +
      q.uid +
      "'" +
      " AND of.sup_uid=sup.uid AND cus.email=ord.cusuid AND ord.cusuid='" +
      q.cusuid +
      "' AND ord.date='" +
      q.date +
      "'" +
    ...
    this.mysql_con.query(sql, (err, sel) => {
      ...
      this.mysql_con.query(sql, values, function (err, result) {
        ...
        if (
          global.resObj[q.cusuid] &&
          global.resObj[q.cusuid].connection.writable
        ) {
          sel[0].status = q.status;
          global.resObj[q.cusuid].send(
            urlencode.encode(
              JSON.stringify({
                func: q.func,
                order: sel[0],
  ...
  ShareLocation(q, ws, cb) {
    let sql =
      ' SELECT sup.email as email' +
      ' FROM ' +
      q.user.toLowerCase() +
      ' as sup' +
      ' WHERE' +
      " SPLIT_STR(sup.region,',',1)<'" +
      q.location[1] +
      "' AND SPLIT_STR(sup.region,',',2)>'" +
      q.location[1] +
      "'" +
      " AND SPLIT_STR(sup.region,',',3)<'" +
      q.location[0] +
      "' AND SPLIT_STR(sup.region,',',4)>'" +
      q.location[0] +
      "'" +
      ' UNION' +
      ...
      ' FROM customer as cus' +
      ' WHERE ' +
      " SPLIT_STR(cus.region,',',1)<'" +
      q.location[1] +
      "' AND SPLIT_STR(cus.region,',',2)>'" +
      q.location[1] +
      "'" +
      " AND SPLIT_STR(cus.region,',',3)<'" +
      q.location[0] +
      "' AND SPLIT_STR(cus.region,',',4)>'" +
      q.location[0] +
      "'";
  ...
  UpdateOrder(q, ws) {
    ...
    let sql =
      ...
      " WHERE del.uid=ord.supuid  AND ord.supuid='" +
      q.supuid +
      "'  AND ord.cusuid='" +
      q.cusuid +
      "'" +
    ...
    this.mysql_con.query(sql, function (err, sel) {
      ...
      sql =
        'UPDATE orders SET  number=?, comment=?, published=?, status=? WHERE cusuid=? AND supuid=? AND date=?';
      ...
      that.mysql_con.query(sql, values, function (err, result) {
        ...
        if (
          global.resObj[q.supuid] &&
          global.resObj[q.supuid].readyState === 1
        ) {
          delete q.uid;
          delete q.func;
          delete q.proj;
          resObj[q.supuid].send(
            urlencode.encode(
              JSON.stringify({
                func: q.func,
                order: q,
        ...
        try {
          ...
          if (sel[0] && sel[0].email) {
            for (let i in q.data) {
              ...
              // return;
              if (
                JSON.parse(sel[0].dict).dict[i] &&
                JSON.parse(sel[0].dict).dict[i]['ru']
              )
                order +=
                  ++cnt +
                  '. ' +
            ...
            let em = new Email();
            let html =
              'По вашему предложению был сформирован заказ:<br> ' +
              order +
              "<p>Для подтверждения заказа перейдите  <a href='https://delivery-angels.ru/d2d/dist/supplier.html?lang=ru&order_date=" +
              sel[0].date +
              "'>по ссылке</a>";
            ...
            em.SendMail(
              'nedol@yandex.ru',
              sel[0].email,
              'ДоТуДо. Оповещение о новом заказе',
              html,
              function (result) {}
            );
          }
