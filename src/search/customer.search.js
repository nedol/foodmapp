...
$(document).on('readystatechange', function () {
  ...
  window.InitCustomerSearch = function (data) {
    if (!window.cus_search) window.cus_search = new CustomerSearch();
    ...
    for (let i in data) {
      ...
      $(
        "<tr style='text-align: center;'>" +
          ...
          dict.getValByKey(window.parent.sets.lang, sup.obj.title) +
          '</td>' +
          '<td>' +
          sup.profile.name +
          '</td>' +
          '<td>' +
          (
            utils.LatLonToMeters(gps[1], gps[0], sup.loc[1], sup.loc[0]) / 1000
          ).toFixed(0) +
          ' км</td>' +
          "<td  style='margin: 10px'>" +
          Object.keys(sup.obj.packlist)[0] +
          ':' +
          sup.obj.packlist[Object.keys(sup.obj.packlist)[0]] +
          '</td>' +
          '<td></td>' +
          '</tr>'
      ).appendTo($('tbody'));
