...
class UtilsMap {
  ...
  IsInsideRadius(map, feature) {
    let radius = feature.values_.object.radius;
    let center = feature.values_.geometry.flatCoordinates;
    ...
    let dist = //utils.getCoordsDistance( that.map.ol_map,center,event.coordinate);
      this.getDistanceFromLatLonInKm(
        proj.toLonLat(center)[1],
        proj.toLonLat(center)[0],
        proj.toLonLat(map_center)[1],
        proj.toLonLat(map_center)[0]
      );
    if (feature.values_.object && feature.values_.object.uid)
      if (dist * 1400 <= radius) {
        return true;
      }
