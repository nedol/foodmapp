  ...
  window.db = new DB('Customer', function () {
    ...
    window.db.GetSettings(function (set) {
      ...
      if (set[res]) {
        ...
        window.user.InitUser(function (data) {
          //TODO:
        });
      } else {
        let md5 = require('md5');
...
export class Customer {
  ...
  InitUser(cb) {
    ...
    $('.profile_frame').attr(
      'src',
      './customer/profile.customer.' + window.sets.lang + '.html'
    );
    $('.cart_frame').attr(
      'src',
      './customer/cart.customer.' + window.sets.lang + '.html'
    );
  ...
  InitStoreOrder(obj, targ_title) {
    ...
    $('#splash')
      .find('img')
      .attr('src', that.image_path + obj.profile.avatar);
    ...
    function load() {
      ...
      client_frame.attr(
        'src',
        './customer/store.html?v=' + new Date().valueOf()
      );
