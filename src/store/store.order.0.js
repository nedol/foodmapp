...
import { CustomerOrder } from '../customer/customer.order.frame.js';
...
$(document).on('readystatechange', function () {
  ...
  if (window.parent && window.parent.sets.css)
    $('#cus_link').attr(
      'href',
      '../css/' + window.parent.sets.css + '.css?v=' + window.parent.v
    );
...
export class StoreOrder extends CustomerOrder {
  constructor() {
    super();
  ...
  openFrame(obj, targ_title, cb) {
    ...
    if ($('.kolmit').length === 0) {
      ...
      $(kolmit)
        ...
        .attr(
          'src',
          '../kolmitt/iframe.html?abonent=user&em=' + obj.profile.email
        );
    ...
    function initOrder(targ_title) {
      ...
      function openItem(cat_tab, i) {
        ...
        if (
          that.offer[cat_tab][i].prop ||
          that.offer[cat_tab][i].content_text
        ) {
            ...
            for (let p in that.offer[cat_tab][i].prop) {
              ...
              if (
                $('#tab_' + cat_tab)
                  .find('.filter_div')
                  .find(
                    'label:contains(' + that.offer[cat_tab][i].prop[p] + ')'
                  )[0]
              )
                continue;
              let cpy = $(el).clone();
        ...
        let setPrice = function (packlist, mi) {
          ...
          for (let p in pl) {
            ...
            if (pl[p].price)
              data =
                window.parent.sets.currency === '₽'
                  ? parseFloat(pl[p].price) + ml_val
                  : (parseFloat(pl[p].price) + ml_val).toFixed(2);
            else
              data =
                window.parent.sets.currency === '₽'
                  ? parseFloat(pl[p]) + ml_val
                  : (parseFloat(pl[p]) + ml_val).toFixed(2);
        ...
        if (
          ...
          $(menu_item)
            ...
            .on('click', { off: that.offer[cat_tab][i] }, function (ev) {
              ...
              let price = {
                ru: pl[$(ev.target).text()] ? pl[$(ev.target).text()] : '',
                en: pl[$(ev.target).text()] ? pl[$(ev.target).text()] : '',
              }[window.parent.sets.lang];
              if (that.offer[cat_tab][i].bargain === 'true') {
                $(this)
                  .closest('.menu_item')
                ...
                { scroll: true },
                {
                  create: function (event, ui) {
                    $(menu_item)
                      .find('.cert_container')
                      .find('img')
                      .css('left', that.offer[cat_tab][i].cert[c].left);
                    $(menu_item)
                      .find('.cert_container')
                      .find('img')
                      .css('top', that.offer[cat_tab][i].cert[c].top);
                  },
                  start: function (ev) {},
                  drag: function (ev) {
                    return true;
