...
export class CustomerOrderFrameEditor {
  ...
  openFrame(obj, targ_title, cb) {
    ...
    $('#delivery_address').parent().css('display', 'none');
    ...
    $('a[data-toggle="tab"]').on('click touchstart', (ev) => {
      ...
      if ($('#delivery_address').val().includes(';'))
        window.parent.user.map.geo.SearchPlace(
          $('#delivery_address').val().split(';'),
          19,
          function (res) {
            $('#delivery_address').val(res.street + ',' + res.house);
    ...
    this.ovc
      ...
      .on('click touchstart', this, async function (ev) {
        ...
        if (
          ...
          $('#delivery_address').parent().css('display', 'block');
          ...
          $('#address_loc').on('click', this, (ev) => {
            ...
            if (
              parseInt(
                $('#customer_frame', window.parent.document).css('height')
              ) < 200
            ) {
              $('#customer_frame', window.parent.document).css(
                'height',
                '100%'
              );
                  ...
                  function (res) {
                    try {
                      if (res.street) {
                        ...
                        window.parent.user.orders[0].address =
                          res.street + ',' + res.house;
                        window.parent.user.profile.profile.address =
                          res.street + ',' + res.house;

                        $('#delivery_address').focus();
                        //$('#delivery_address')[0].select();
                        $('#delivery_address').trigger('click');
                      }
                      return false;
                    } catch (ex) {}
                  }
              ...
              if (!loc) loc = JSON.parse(localStorage.getItem('cur_loc'));
              ...
              window.parent.user.map.MoveToLocation(loc, null, () => {
                $(window.parent.user.map.ol_map).on('moveend', (ev) => {
                  ...
                  $('#delivery_address').attr(
                    'loc',
                    JSON.stringify([loc[1], loc[0]])
                  );
                  $('#delivery_address').val(
                    String(loc[1].toFixed(6)) + ';' + String(loc[0].toFixed(6))
                  );
                  window.parent.user.profile.profile.loc =
                    $('#delivery_address').val();
                });
              });
            }
        ...
        function save() {
          ...
          that.SaveOrder(items, function (res) {
            if (res) {
              $('.filter_div').remove();
              $('.loader').css('display', 'none');
              if ($('.kolmit')[0].contentWindow.CloseFrame)
                $('.kolmit')[0].contentWindow.CloseFrame();
              $(frameElement).css('display', 'none');
              $('#customer_frame', window.parent.document).css(
                'display',
    ...
    window.parent.db.GetSettings(function (obj) {
      if (obj[0].profile && obj[0].profile.address)
        that.ovc.find('#delivery_address').val(obj[0].profile.address);
    });
    ...
    async function initOrder(targ_title) {
      let promise = new Promise((resolve, reject) => {
        ...
        for (let t in that.offer) {
          openTab(t);
          if (Object.keys(that.offer).length - 1 === i) resolve(t);
          i++;
        }
      });
      ...
      function openItem(cat_tab, i) {
        ...
        if (that.offer[cat_tab][i].content_text) {
          if (
            ...
            if (str && str.trim().length > 0) {
              ...
              $(menu_item).find('.item_desc').css('visibility', 'visible');
              $(menu_item)
                .find('.item_desc')
                .attr('data-target', '#content_' + cat_tab + '_' + i);
            } else {
              $(menu_item).find('.item_desc').css('visibility', 'hidden');
            }
          }
        }
        ...
        function CheckDiscount(pl, prev_pl, p) {
          ...
          if (Math.floor(((prev_price - price) / price) * 100) >= 1) {
            $(menu_item).find('.item_price').text(prev_price).attr('dscnt', '');
            $(menu_item)
              .find('.discount')
              .css('display', '')
              .text(
      ...
      window.parent.db.GetSupOrders(that.date, obj.uid, function (arObj) {
        if (arObj.length > 0) {
          for (let o in arObj) {
            ...
            // } else {
            //   if (that.address)
            //     $(that.ovc).find('#sup_adr').val(that.address);
            // }

            // $(this).siblings('.ord_amount').text(amnt);

            if (order.status.checked || order.status.ordered) {
              for (let title in order.data) {
                let mi = $('[data-translate=' + title + ']').closest(
                  '.menu_item'
                );
                let cur_pack = $(mi).find('.pack_btn').text();
                mi.find('.dropdown-item:contains(' + cur_pack + ')').attr(
                  'ordlist',
                  JSON.stringify(
                    order.data[title].ordlist[
                      Object.keys(order.data[title].ordlist)[0]
                    ]
                  )
                );
                if (
            ...
            if (order.status.published) {
              that.published = order.status.published;
            ...
            if (order.comment) {
              $(that.ovc)
                .find('.comment')
                .text(that.dict.getDictValue(window.sets.lang, order.comment));
            }
          }
  ...
  SaveOrder(items, cb) {
    ...
    if ($('#delivery_address').val()) {
      window.parent.db.GetSettings((obj) => {
        ...
        //obj[0].profile.address = $('#delivery_address').val();
