...
window.InitCartCustomer = function () {
    ...
    if(!window.cart_cus)
        window.cart_cus = new CartCustomerStore();
            ...
            let promise = new Promise((resolve, reject) => {
                ...
                window.parent.db.GetSupplier(window.parent.user.date, order.supuid, async function (sup) {
                    ...
                    let sel_period =
                        ...
                        "<input type='date' id='ts_date' style='width:100%;border-width: 0px;border-bottom-width: 0px;'>"+
                    ...
                    for (let item in order.data) {
                        ...
                        if (sup != -1) {
                            ...
                            for (let o in order.data[item].ordlist) {
                                ...
                                if (ord.qnty !== 0) {
                                            ...
                                            (sup.profile.type === 'deliver' ?
                                                    ("<textarea type='text' class='form-control delivery_adr' style='width:300px;' placeholder= '"+
                                                        (!window.parent.user.profile.profile.address?
                                                            ...
                                                            window.parent.user.profile.profile.address)
                                                        +"'>"+window.parent.user.profile.profile.address+"</textarea>")
                                                    : ("<textarea type='text' readonly class='form-control sup_adr' style='width:300px;'>"+(sup.profile.place!==''?sup.profile.place:(sup.latitude+';'+sup.longitude))+"</textarea>")
                                            ) +
                                            "</td>" +
                                            "</tr>"
                                        ).appendTo($('tbody'));
                                    ...
                                    let src = _.find(sup.data[cat], { 'title': item }).cert[0].src;
                                    ...
                                    const insert = $("<tr class='order_item'" +
                                        ...
                                        "<div>" + o + "</div>" +
                                        "<div>" +
                                        "<img class='col' src='"+that.path+'images/'+src+"' style='max-width: 80px; max-height: 80px'>" +
                                        "</div>" +
                                        "</div>" +
                                        "</td>" +
                                        ...
                                        "<textarea  class='tacomment' rows='4' style='width: 95%' placeholder='"+window.parent.sysdict.getValByKey(window.parent.lang,'a5d491060952aa8ad5fdee071be752de')+"'>" +
                        ...
                        $('tr[supuid="' + order.supuid + '"]:first .comments').attr('rowspan', $('tr[supuid="' + order.supuid + '"]').length-1);
                        ...
                        if($('.delivery_adr').val()) {
                            ...
                            let promise = new Promise(function (res,reject) {
                                ...
                                if(isFloat(parseFloat(adr[0])) && isFloat(parseFloat(adr[1]))){
                                    ...
                                    window.parent.user.map.geo.SearchLocation(adr[0], function (loc) {
                                        loc_1 = loc;
                                        if(loc_1) {
                            ...
                            if(dist && dist>sup.radius/1000){
                                ...
                                del_insert = $(
                                    ...
                                    "</tr>");
                                del_insert.insertBefore($("tr.pay_supuid[supuid='" + order.supuid + "']"));
                                resolve();
                                return;
                            }
                            $('[tr_delivery]').remove();
                            ...
                            if($('[tr_delivery]')[0])
                                $('[tr_delivery]').remove();
                            $('.publish_order', $('tr.pay_supuid[supuid="' + order.supuid + '"]')).css('display','');
                            $("tr.pay_supuid[supuid='" + order.supuid + "']").find('td.ord_sum').text(sum+del_cost);
                            del_insert.insertBefore($("tr.pay_supuid[supuid='" + order.supuid + "']"));

                        }else {
                            $("tr[supuid='" + order.supuid + "']").find('.delivery_adr')[0].scrollIntoView()
                        }
