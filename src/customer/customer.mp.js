...
export class MPCustomer {
  constructor(type) {
    ...
    $('#mp_frame_div').load(
      './customer/mp.customer.html?v=' + new Date().getTime(),
      async function () {
          ...
          window.db.GetApproved(
            ...
            function (res) {
              if (res && res.data[title]) {
                ...
                if (
                  !$(
                    '.cat_div[href="#tab_' + $(tr).attr('category') + '"]',
                    $('.mp_categories')
                  ).find('.mp_appr_cnt')[0]
                )
                  $(
                    ...
                    '<span class="mp_appr_cnt badge badge-pill badge-success">' +
                      1 +
                      '</span>'
                  );
                else {
              ...
              for (let title in res.data) {
                ...
                if (res.data && res.data[title] && res.data[title]['ordlist']) {
                  ...
                  for (let i in res.data[title]['ordlist']) {
                    ...
                    if (ord.qnty === 0) continue;
                    tr.find('.dropdown-item:contains(' + i + ')').attr(
                      'order',
                      JSON.stringify(ord)
                    );
                    tr.find('.ord_amount')
                    ...
                    if (!$(tr).hasClass('approved')) {
                      ...
                      if (cnt) {
                        ...
                        $(
                          ...
                          '<span class="mp_ord_cnt badge badge-pill badge-danger">' +
                            cnt +
                            '</span>'
                        );
                      }
        ...
        that.dialog = $('#popup').dialog({
          ...
          open: function (event, ui) {
            ...
            $('.mp')
              .tablesorter({
                theme: 'materialize', //'dropbox', //'grey', //'blue', //'bootstrap', //'jui', //
                headers: {
                ...
                widgetOptions: {
                  ...
                  pager_output:
                    '{startRow:input} – {endRow} / {totalRows} ' +
                    window.sysdict.getDictValue(window.sets.lang, 'записей'), // '{page}/{totalPages}'
              ...
              .bind(
                'pagerChange pagerComplete pagerInitialized pageMoved',
                function (e, c) {
  ...
  async Close(cb) {
    ...
    if (md5(JSON.stringify(this.items)) !== md5(JSON.stringify(items))) {
      if (
        that.type === 'deliver' &&
        !window.user.profile.profile.address &&
        ...
        $('#address_loc').on('click', this, (ev) => {
          ...
          if ($('#popup').css('display') === 'none') {
            ...
            if ($('#address').val().includes(';')) {
              ...
              window.user.map.geo.SearchPlace(
                $('#address').val().split(';'),
                zoom,
                function (res) {
                  try {
  ...
  InitOffers(resolve) {
    ...
    for (let l in layers.array_) {
      ...
      if (layer.type === 'VECTOR' && layer !== layers.circleLayer) {
        ...
        _.each(f_s, function (f) {
          if (f.values_.object && f.values_.object.profile.type === 'deliver') {
            if (util.IsInsideRadius(window.user.map, f)) {
              features.push(f.values_.object);
            }
          }
        });
      }
    ...
    window.db.GetRangeSupplier(
      ...
      (f_s) => {
        if (f_s[0]) features = _.unionBy(features, f_s, 'uid');
          ...
          for (let o in features[f].data) {
            ...
            let addTab = function (cat_tab, cat_img, active) {
              ...
              if ($('#' + cat_tab).closest('.cat_div')[0])
                ...
                cat_str =
                  ...
                  '</div>';
              if (
                $('[href="#tab_' + cat_tab + '"]', $('#popup')).length === 0
              ) {
            ...
            let setPrice = function (index, packlist, prev_pl, el) {
              ...
              for (let p in pl) {
                ...
                if (pl[p].price)
                  data = parseFloat(
                    pl[p].price.replace(/[^.-a-z0-9+]+/gi, '')
                  ).toFixed(2);
                else data = parseFloat(pl[p]).toFixed(2);
                ...
                if (
                  !$(
                    '.carousel_price[title=' +
                      features[f].data[cat_tab][index].title +
                      ']'
                  ).text()
                )
                  $(
              ...
              $(el)
                ...
                .on('click', { pl: pl }, function (ev) {
                  ...
                  $(this)
                    ...
                    .text($(ev.target).text());
                  let pl = ev.data.pl;
                  let price = parseFloat(pl[$(ev.target).attr('pack')]).toFixed(
                    2
                  );
                  ...
                  let ord = $(this)
                    .closest(
                      'tr .dropdown-item:contains(' + $(ev.target).text() + ')'
                    )
                    .attr('order');
                  if (ord) {
                    ord = JSON.parse(ord);
            ...
            for (let i in features[f].data[o]) {
              ...
              if (features[f].uid && !$('tr.' + features[f].uid)[0]) {
                ...
                tr.find('div[type] img')
                  .attr(
                    'src',
                    features[f].profile.avatar
                      ? that.image_path + features[f].profile.avatar
                      : ''
                  )
                  .on('click', function () {
                ...
                tr.find('td[title] div:not([type])').prepend(
                  dict.getValByKey(
                    window.sets.lang,
                    item.title,
                    features[f].profile.lang
                ...
                tr.find('td[title] div:not([type])').attr(
                  'data-translate',
                  item.title
                );
                if (
                  ...
                  tr.find('td[image] button[data-toggle="collapse"]').css(
                    'display',
                    'block'
                  );
                  tr.find('td[image] button[data-toggle="collapse"]').on(
                    'click touchstart',
                    function () {
                      $(this)
                        .siblings('.img_container')
                        .find('.collapse')
                        .collapse('toggle');
                    }
                  );
                ...
                tr.find('td[image].content_text').append(
                  dict.getValByKey(
                    window.sets.lang,
                    item.content_text.value,
                    features[f].profile.lang
                ...
                tr.find('td[image].content_text').on(
                  'click touchstart',
                  function (ev) {
                    //$(this).collapse('hide');
                  }
                ...
                if (item.img) {
                  if (item.img.src.includes('http'))
                    tr.find('td[image] img').attr(
                      'src',
                      item.cert[0] ? item.cert[0].src : item.img['src']
                    );
                  else
                    tr.find('td[image] img').attr(
                      'src',
                      that.image_path + item.img['src']
                    );
                } else if (item.cert[0]) {
                  if (item.cert[0].src.includes('http'))
                    ...
                    tr.find('td[image] img').attr(
                      'src',
                      that.image_path + item.cert[0].src
                    );
                }
                ...
                //     tr.find('td[type] img').attr('src', features[f].profile.avatar?that.image_path  + features[f].profile.avatar:'').on('click', function () {
                //         if (window.user.constructor.name === 'Customer') {
                //             if (!window.user.viewer) {
                //                 window.user.viewer = new OfferOrder();
                //             }
                ...
                else tr.prependTo($('.thidden', $('#popup')));

                let spltr = $('.tr_splitter_tmplt', $('.mp')).clone();
                spltr.css('display', 'block');
                spltr.removeClass('tr_splitter_tmplt');
                spltr.addClass('tr_splitter');
                spltr.insertAfter(tr);
            ...
            $('.cat_div #' + o, $('#popup'))
              .siblings('.items_cnt')
              .text($('tr[category=' + o + ']', $('#popup')).length);
          }
        ...
        $('.increase_ord').on('click', function (ev) {
          ...
          if (!price) price = $(tr).find('.item_price').attr('placeholder');
  ...
  GetOrderItems() {
    ...
    $('tr[supuid]').each(function (i, el) {
      ...
      if (Object.keys(ordlist).length === 0) return;
  ...
  SetSlider_() {
    ...
    let step = parseInt((max - min) / 100);
    try {
      let sl = $('#slider').slider({
        classes: { 'ui-slider': 'highlight' },
        value: 35,
        step: 1,
      });
      sl.slider('option', 'min', 10).slider('option', 'max', 50);
    } catch (ex) {
      console.log(ex);
    }
