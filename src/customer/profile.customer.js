...
class ProfileCustomer{
    constructor(tab){
        ...
        $('a[role=langitem]').on('click',function () {
            ...
            window.parent.location.replace("../customer.html?lang="+ $(this).text()+"&market=food&css=order.3");
        });
        ...
        $('#close_browser').on('click', this, function (ev) {
            ...
            that.Close(()=>{
                $('#profile_frame',$(window.parent.document).contents()).css('display', 'none');
    ...
    fillProfileForm(cb){
        ...
        if(data && data.profile) {
            ...
            for (let i in data.profile) {
                if (i === 'avatar') {
                    if(!data.profile[i].includes('http') && !data.profile[i].includes('data')) {
                        let src = that.path+'/images/' +data.profile[i];
                        $('.avatar').attr('src', src);
                    }else{
        ...
        for(let i in arrInp){
            if($(arrInp[i]).attr('type')==='file'){
                if($(arrInp[i]).siblings('img').attr('src').includes('http')) {
                    ...
                    profile['avatar'] = $(arrInp[i]).siblings('img').attr('src');
            }else
                profile[arrInp[i].id] = $(arrInp[i]).val();
        }
        return profile;
    }
