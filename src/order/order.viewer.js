...
class OrderViewer {
  ...
  InitOrders(objs) {
    ...
    $(".category[state='1']").each(function (i, cat) {
      let tab = cat.title;
      $(
        '<li class="tab_inserted"><a cat="' +
          cat.id +
          '" data-toggle="tab" contenteditable="true"  data-translate="' +
          md5(tab) +
          '"  href="#' +
          tab +
          '"' +
          ' style="color:grey;margin:1px;">' +
          cat.title +
          '</a>' +
          '</li>'
      ).insertBefore($('#orders_add_tab_li'));
      $(
        '<div id="' +
          tab +
          '" class="tab-pane fade div_tab_inserted" style="border: none">' +
          '</div>'
      ).insertBefore($('#orders_add_tab_div'));
    ...
    function fillTableOrders(tbl, tab) {
      for (let c in that.orders) {
        ...
        for (let o in data) {
          ...
          if (data[o].cat !== tab) continue;

          let checked = objs[c].data[o].status === 'approved' ? 'checked' : '';
          let cbdisabled = '';
          if (objs[c].data[o].status === 'approved') cbdisabled = 'disabled';
          let row =
            "<tr style='text-align: center;'>" +
            "<td class='tablesorter-no-sort'>" +
            '<label  class="btn">' +
            '<input type="checkbox" class="approve checkbox-inline" ' +
            cbdisabled +
            ' cusuid=' +
            objs[c].cusuid +
            ' ' +
            ...
            data[o].qnty +
            '</td>' +
            "<td onclick='window.user.viewer.OnClickAddress(that)'>" +
            objs[c].address +
            '</td>' +
            '<td>' +
            objs[c].dist +
    ...
    $(this.ordv)
      .find('.close_browser')
      .on('click touchstart', this, that.Close);
    // let sp = $('.sp_dlg');
    // $(sp).selectpicker();
