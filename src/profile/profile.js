...
class Profile {
    ...
    browser.on('load', function () {
      ...
      if (that.profile) {
        ...
        if (!that.profile.email) {
          $('#email', browser.contents()).attr(
            'placeholder',
            'Не подтвержден. Проверьте почту'
          );
        } else {
      ...
      $('.close_browser', browser[0].contentWindow).on(
        'click touchstart',
        function (ev) {
  ...
  LoadSupplierProfile(that, ovc, rating) {
    ...
    browser.off('load');
    browser.on('load', function () {
      browser[0].contentWindow.InitProfileSupplier(
        { supuid: that.uid, user: window.user.constructor.name },
        {
          //comments settings
          readOnly:
            that.appr && that.appr.cusuid === window.user.uid ? false : true,
          profilePictureURL: that.profile.thmb
            ? that.profile.thmb
            : that.profile.avatar,
          enableEditing: true,
          enableDeleting: false,
      ...
      if (!window.user.profile.profile.type)
        $('a[href="#settings"]', browser.contents())
          .parent('li')
          .css('display', 'none');
      ...
      $('.close_browser', browser.contents()).on(
        'click touchstart',
        function (ev) {
  ...
  SaveProfile() {
    let browser = $('#profile_container').find('.browser');
    browser[0].contentWindow.profile_sup.SaveProfile(
      window.user.uid,
      window.user.psw
    );
  }
