...
class OfferEditor {
  constructor() {
    ...
    this.off_frame.attr(
      'src',
      './supplier/supplier.frame.' + window.sets.lang + '.html?v=' + v
    );
  }
  ...
  InitSupplierOffer() {
    ...
    if (window.dict) this.offer.dict = window.dict.dict;
    this.offer.promo = window.user.promo;
    this.offer.prolong = window.user.settings.prolong;
  ...
  OnClickAddCert(ev) {
    ...
    $(menu_item)
      ...
      .attr(
        'func_el',
        JSON.stringify({ func: 'add_cert', id: $(ev.target).attr('id') })
      );
  ...
  AddOfferItem(ev) {
    ...
    $(menu_item)
      .find('.item_content')
      .attr('id', 'content_' + tab.replace('#', '') + pos);
    $(menu_item)
      .find('.item_title')
    ...
    $(menu_item)
      ...
      .on('click touchstart', menu_item, function (ev) {
        ...
        $(menu_item)
          .find('.toolbar')
          .insertAfter($(menu_item).find('.item_content'));
      });

    $(menu_item)
      .find('.item_price')
      .on('focusout', { that: that, mi: $(menu_item) }, function (ev) {
    ...
    $(menu_item)
      .find('.align_center_bottom')
      .click(function (ev) {
        let pos =
          $(menu_item).find('.img-fluid').parent().width() / 2 -
    ...
    $(menu_item)
      .find('.add_cert')
      .attr('id', 'ac_' + tab.replace('#', '') + '_' + pos);
    $(menu_item)
      .find('.add_cert')
      .on('click', this, function (ev) {
    ...
    $(menu_item)
      .find('.add_pack')
      .attr('id', 'pack_' + tab.replace('#', ''));
    $(menu_item)
      .find('.add_pack')
      .on('click', { mi: $(menu_item), that: that }, that.OnClickAddPack);
      ...
      .each((index, val) => {
        ...
        for (let i = 0; i < miAr.length; i++) {
          ...
          item.checked = JSON.stringify(
            $(miAr[i]).find(':checkbox').prop('checked')
          );
          ...
          if (
            $(miAr[i]).find('.content_text').css('visibility') === 'visible'
          ) {
          ...
          if ($(miAr[i]).find('.img-fluid').css('visibility') === 'visible') {
            ...
            item.img.left = String(left).includes('%')
              ? (parseInt(left) / 100) * item.width
              : parseInt(left);
          ...
          $.each(
            $(miAr[i]).find('.orders').find('input:checkbox:checked'),
            function (i, el) {
              window.db.GetOrder(
                window.user.date,
                window.user.uid,
  ...
  CloseMenu(ev) {
    ...
    if (that.changed) {
      if (
        $('.menu_item').find('input:checked[tab]').length > 0 &&
        window.user.ValidateOffer(items['remote'])
      ) {
  ...
  OnChangeLang(ev) {
    ...
    let sel_lang = $('.sp_dlg option:selected')
      .val()
      .toLowerCase()
      .substring(0, 2);
