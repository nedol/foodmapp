...
class Geo {
    ...
    if ('geolocation' in navigator) {
      ...
      navigator.geolocation.getCurrentPosition(
        function (position) {
          ...
          if (coordinates) window.postMessage(coordinates, '*'); //http://delivery-angels.ru');
        },
  ...
  ChangeGPSPosition(that, coor) {
    ...
    if (window.sets.coords.cur === coor) return;
    ...
    if (window.user.SendLocation) window.user.SendLocation(coor);
  }
  ...
  SetCurPosition(that, coor) {
    try {
      ...
      if (
        window.sets.loc_mode &&
        (that.map.ol_map.getView().getCenter()[0] !== coor[0] ||
  ...
  SearchLocation(place, cb) {
    ...
    let locationiq = 'https://locationiq.org/v1/search.php';
  ...
  SearchPlace(latlon, zoom, cb) {
    ...
    let reverse =
      ...
      '&zoom=' +
      zoom +
      '&addressdetails=2&accept-language=' +
      window.sets.lang;
    let mapques =
      'https://open.mapquestapi.com/geocoding/v1/reverse?key=KEY&location=30.333472,-81.470448&includeRoadMetadata=true&includeNearestIntersection=true';
    let locationiq = 'https://locationiq.org/v1/search.php';
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        let obj = {
          ...
          house: data.address.house_number ? data.address.house_number : '',
        };
        cb(obj);
      },
      error: function (data) {
  ...
  GetDistanceToPlace(loc, place, cb) {
    ...
    this.SearchLocation(place, function (bound, lat, lon) {
      if (!bound) cb('undefined');
      else cb(new Utils().LatLonToMeters(lonlat[1], lonlat[0], lat, lon));
    });
  }
}
