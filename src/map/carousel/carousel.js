        ...
        try {
            ...
            map.ol_map.on('moveend', function (event) {
                ...
                if(window.user.import.data_updated) {
                    map.GetObjectsFromStorage([tr_ext[1],tr_ext[3],tr_ext[0],tr_ext[2]]);
    ...
    async SetFeatures(features) {
        ...
        for (let f in features) {
            ...
            for (let cat_tab in features[f].data) {
                if ($(".category[id=" + cat_tab + "]", $('#category_container')).length > 0) {
                    let cat_cnt = $('img[id="' + cat_tab + '"]', $('#category_container')).parent().find('.cat_cnt').text()
                    $('img[id="' + cat_tab + '"]', $('#category_container')).parent().find('.cat_cnt').text(parseInt(cat_cnt) + 1)

                    main_cnt = parseInt($($('img[id="' + cat_tab + '"]', $('#category_container')).closest('.dropdown-menu').siblings('.cat_cnt')[0]).text());
                    $($('img[id="' + cat_tab + '"]', $('#category_container')).closest('.dropdown-menu').siblings('.cat_cnt')[0]).text(main_cnt + 1);
                }
                if ($(".category[state='1'][id=" + cat_tab + "]", $('#category_container')).length === 0)
                    continue;
            ...
            for (let tab in features[f].data) {
                ...
                $('.mp_open[user_type="'+features[f].profile.type+'"]').prop('disabled', false);
                ...
                for (let i in  features[f].data[tab]) {
                    ...
                    let title = dict.getValByKey(window.sets.lang, item.title, features[f].profile.lang);
                    ...
                    try {
                        price = //(!window.sets.currency ? '' : window.sets.currency) +
                            parseFloat(Object.values(item.packlist)[0].price ? Object.values(item.packlist)[0].price : Object.values(item.packlist)[0]);
                    } catch (ex) {
                    ...
                    let promise = new Promise((resolve, reject) => {
                        ...
                        $('<div/>').load('./html/carousel.item.html', function (data) {
                            ...
                            if(prev_price && (prev_price - price >0)) {
                                ...
                                $(html).find('.discount').css('display','').text(('-')+Math.floor((((prev_price - price)/price)*100))+'%');
                                $(html).find('.dscnt_price').css('visibility','visible').text(price);
                                active = '';
        ...
        if(that.htmlAr['delivery'].length==0 && that.htmlAr['pickup'].length==0) {
        ...
        for(let tab in that.htmlAr) {
            ...
            if (that.htmlAr[tab].length > 0) {
                ...
                $('#' + tab).find('.carousel-inner').append(shuffled);
                that.shuffled[tab] = shuffled;

                $('#' + tab).find('.carousel-inner').find('.carousel-item').removeClass('active');
                $($('#' + tab).find('.carousel-inner').find('.carousel-item')[0]).addClass('active');

                if (tab === 'delivery') {

                    if(that.htmlAr['pickup'].length===0)
                        $('.nav-link[href="#pickup"]', $('#items_carousel')).addClass('disabled');
                    else {
                        $('.nav-link[href="#pickup"]', $('#items_carousel')).removeClass('disabled');
                    ...
                    if(that.htmlAr['delivery'].length===0)
                        $('.nav-link[href="#delivery"]', $('#items_carousel')).addClass('disabled');
                    else {
                        $('.nav-link[href="#delivery"]', $('#items_carousel')).removeClass('disabled');
