...
class DB {
  constructor(user, f) {
    ...
    if (!window.indexedDB) {
      alert(
        'Ваш браузер не поддерживат стабильную версию IndexedDB. Некоторые функции будут недоступны'
      );
    }
    ...
    if (!this.DBcon) {
      this.connectDB(function (con) {
        let first = false;
        if (!DB.prototype.DBcon) first = true;
    ...
    try {
      ...
      request.onupgradeneeded = function (e) {
        ...
        try {
          ...
          let vSetStore = db.createObjectStore(that.settingsStore, {
            keyPath: 'uid',
          });
        ...
        try {
          ...
          let vOfferStore = db.createObjectStore(
            that.offerStore,
        ...
        try {
          ...
          let vDictStore = db.createObjectStore(that.dictStore, {
            keyPath: 'hash',
          });
        ...
        try {
          ...
          let vSupplierStore = db.createObjectStore(that.supplierStore, {
            keyPath: ['date', 'uid'],
          });
        ...
        try {
          ...
          let vOrderStore = db.createObjectStore(that.orderStore, {
            keyPath: ['date', 'supuid', 'cusuid'],
          });
          vOrderStore.createIndex('date', 'date', { unique: false });
          vOrderStore.createIndex(
            'datesupuidcusuid',
            ['date', 'supuid', 'cusuid'],
            { unique: true }
          );
          vOrderStore.createIndex('datesupuid', ['date', 'supuid'], {
            unique: false,
          });
        ...
        try {
          ...
          let vApprStore = db.createObjectStore(that.apprStore, {
            keyPath: ['date', 'supuid', 'cusuid'],
          });
          vApprStore.createIndex('datesupcus', ['date', 'supuid', 'cusuid'], {
            unique: true,
          });
  ...
  SetObject(storeName, obj, f) {
    var objectStore = DB.prototype.DBcon.transaction(
      [storeName],
      'readwrite'
    ...
    if (objectStore) {
      try {
        ...
        console.log('Store:' + storeName + ' obj:' + obj);
      }
    }
  }

  ClearStore(storeName, cb) {
    let objectStoreRequest = DB.prototype.DBcon.transaction(
      [storeName],
      'readwrite'
    )
      .objectStore(storeName)
  ...
  GetStorage(storeName, f) {
    var rows = {},
      store = DB.prototype.DBcon.transaction(
        [storeName],
        'readonly'
      ).objectStore(storeName);
      ...
      if (cursor) {
        ...
        var markerFeature = new ol.Feature({
          geometry: new ol.geom.Point(
            ol.proj.fromLonLat([cursor.value.longitude, cursor.value.latitude])
          ),
  ...
  GetRangeSupplier(date, lat_0, lon_0, lat_1, lon_1, f) {
    ...
    idateuid.openCursor().onsuccess = function (event) {
      ...
      if (cursor) {
        if (
          lat_0 < cursor.value.latitude &&
          lat_1 > cursor.value.latitude &&
          lon_0 < cursor.value.longitude &&
          lon_1 > cursor.value.longitude
        )
          if (cursor.value.date === date) features.push(cursor.value);
      ...
      if (cursor) {
        if (
          ...
          if (cursor.value.date === date)
            if (cursor.value.data[cat]) features.push(cursor.value);
  ...
  DeleteOrder(date, supuid, cusuid, cb) {
    var request = DB.prototype.DBcon.transaction('orderStore', 'readwrite')
      .objectStore('orderStore')
      .delete([date, supuid, cusuid]);
    request.onerror = function (ev) {
      console.log(ev);
    };
    request.onsuccess = function () {
  ...
  GetObject(storeName, date, email, cb) {
    ...
    try {
      let objectStore = DB.prototype.DBcon.transaction(
        storeName,
        'readonly'
      ).objectStore(storeName);
  ...
  DeleteObject(store, uid, cb) {
    var request = DB.prototype.DBcon.transaction(store, 'readwrite')
      .objectStore(store)
      .delete(uid);
    request.onerror = function (ev) {
      console.log(ev);
  ...
  GetOfferTmplt(cb) {
    ...
    objectStoreRequest.onsuccess = function (event) {
      // report the success of our request
      if (objectStoreRequest.result) cb(objectStoreRequest.result);
      else cb();
    };
  }
  ...
  GetAllOffers(cb) {
    ...
    ind.openCursor().onsuccess = function (event) {
      var cursor = event.target.result;
      if (cursor) {
        if (cursor.value.date !== 'tmplt') ar.push(cursor.value);
