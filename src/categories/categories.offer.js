...
export class CategoriesOffer extends Categories{
    ...
    OnClickCategory(ev) {
        ...
        $(el).closest('.dropdown-menu').siblings('.main_category').css('opacity',1).attr('state','1');

        $('.menu_item').css('display','none');
        $('.menu_item[cat='+$(el).attr('id')+']').css('display','')
        $(el).siblings('.cat_cnt').text($('.menu_item[cat='+$(el).attr('id')+']').length);
        let ar = $(el).closest('.cat_div').siblings('.cat_div').find('.cat_cnt')
        var total = 0;
        $.each(ar,function(i, item) {
            total += parseInt($(item).text());
        });
        $(el).closest('.dropup').children('.cat_cnt').text(total+ parseInt($(el).siblings('.cat_cnt').text()));
