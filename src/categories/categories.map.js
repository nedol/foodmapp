...
export class CategoriesMap extends Categories {
  ...
  initCategories() {
    ...
    $('.mp_open').on('click', function (ev) {
      ...
      $('.mp_frame').attr(
        'src',
        './customer/mp.customer.html?v=' + new Date().valueOf()
      );

      $('.loader').css('display', 'block');

      window.user.mp = new MPCustomer(
        $(ev.target).closest('.mp_open').attr('user_type')
      );
    });
  ...
  WordSearch() {
    ...
    $('.word_input').on('focusout', function (ev) {
      ...
      if (
        ...
        $('.word_btn[value="' + ev.currentTarget.value + '"').length > 0
      )
        return true;
...
//////////////////
// WEBPACK FOOTER
// ./src/categories/categories.map.js
// module id = 688
// module chunks = 0
