...
export class CustomerOrder {
  ...
  openFrame(obj, targ_title, cb) {
    ...
    this.ovc
      .find('.actual_price')
      .text(
        { ru: 'Цены на самовывоз : ', en: 'Pick up prices for ' }[
        ...
        if (that.offer[cat_tab][i].img) {
          ...
          if (!that.offer[cat_tab][i].owner) {
            ...
            let item =
              ...
              '<img class="img-fluid img-responsive d-block" src=' +
              src +
              ' alt="slide"  style="width: 100%;height: 300px;object-fit: contain ;"></div>';
          ...
          for (let c in that.offer[cat_tab][i].cert) {
            ...
            if ($(menu_item).find('img[src="' + src + '"]').length === 0)
              $(menu_item)
                ...
                .append(
                  '<div class="carousel-item">' +
                    '<img  class="carousel-img img-responsive img-fluid mx-auto d-block" src=' +
                    src +
                    '>' +
                    '</div>'
                );
