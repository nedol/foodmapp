...
export class SupplierOffer{
    constructor(){
        ...
        window.parent.user.map.ol_map.on('moveend', function (event) {
    ...
    OpenOffer() {
        ...
        $('#lang_sel button').html($('#lang_sel [role=langitem][value='+window.parent.sets.lang+'] span')[0].outerHTML);
    ...
    RedrawOrder(obj){
        let that = this;
        window.parent.db.GetOrder(this.date, obj.uid, window.parent.user.uid, function (res) {
            if(res!==-1){
                ...
                for(let k in keys){
                    if(keys[k]==='comment'){
                        ...
                        window.parent.db.GetApproved(that.date,obj.uid,window.parent.user.uid,keys[k],function (appr) {
                            if(appr &&
    ...
    SaveOffer(items) {
        ...
        // }
        if(items['local'])
            window.parent.user.UpdateOfferLocal(this.offer,items['local'],
                proj.fromLonLat(window.parent.user.offer.stobj.longitude,window.parent.user.offer.stobj.latitude) ,
